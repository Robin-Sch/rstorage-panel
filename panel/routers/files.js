...
router
	.get('/:id/view/:path?', async (req, res) => {
		...
		fetch(`http://${node.ip}:${node.port}/files`, {
			...
			.then(encryptedjson => {
				...
				else res.render('files', { files: json.files, directories: json.directories, node: node._id });
			}).catch(() => {
	...
	.post('/:id/upload', async (req, res) => {
		...
		if (!req.files) return res.json({ message: 'No file selected!', success: false });
		...
		fetch(`http://${node.ip}:${node.port}/upload`, {
			method: 'POST',
			body: JSON.stringify(encryptedbody),
			headers: { 'Content-Type': 'application/json' },
		}).then(res2 => res2.json())
